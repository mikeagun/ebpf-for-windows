...
TEST_CASE("listen_helper_functions_validation_tcp_v4", "[sock_addr_tests][helper_validation]")
{
    ...
    int result = bpf_prog_attach(
        bpf_program__fd(const_cast<const bpf_program*>(listen_program)), 0, BPF_CGROUP_INET4_LISTEN, 0);
    SAFE_REQUIRE(result == 0);
...
TEST_CASE("listen_helper_functions_validation_tcp_v6", "[sock_addr_tests][helper_validation]")
{
    ...
    int result = bpf_prog_attach(
        bpf_program__fd(const_cast<const bpf_program*>(listen_program)), 0, BPF_CGROUP_INET6_LISTEN, 0);
    SAFE_REQUIRE(result == 0);
