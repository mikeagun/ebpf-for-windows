...
// Integration test: confirm a real socket bind drives the replacement sock_addr bind hook
// (BPF_CGROUP_INET4_BIND / BPF_CGROUP_INET6_BIND) and that the program's ring buffer event is
// delivered to user mode through netebpfext. This restores the real-bind -> event coverage that
// the migrated ring/perf convenience tests no longer provide.
TEST_CASE("sock_addr_bind_ringbuf_integration", "[sock_addr_bind][ring_buffer]")
{
    struct bpf_object* object = nullptr;
    fd_t program_fd = ebpf_fd_invalid;
    REQUIRE(
        program_load_helper(
            "sock_addr_bind_ringbuf.sys",
            BPF_PROG_TYPE_CGROUP_SOCK_ADDR,
            EBPF_EXECUTION_NATIVE,
            &object,
            &program_fd) == 0);

    fd_t map_fd = bpf_object__find_map_fd_by_name(object, "bind_events_map");
    REQUIRE(map_fd > 0);

    // Synchronous ring buffer consumer that records the port from each received bind event.
    struct bind_event_context
    {
    ...
    auto ring = ebpf_ring_buffer__new(
        ...
        [](void* ctx, void* data, size_t size) -> int {
            auto* context = reinterpret_cast<bind_event_context*>(ctx);
            if (size >= sizeof(uint16_t)) {
                context->ports.push_back(*reinterpret_cast<uint16_t*>(data));
            }
            return 0;
        },
        &event_context,
    ...
    auto bind_and_expect_event = [&](const char* program_name, ebpf_attach_type_t attach_type, int family) {
        bpf_program* program = bpf_object__find_program_by_name(object, program_name);
        REQUIRE(program != nullptr);
        bpf_link* link = nullptr;
        REQUIRE(ebpf_program_attach(program, &attach_type, nullptr, 0, &link) == EBPF_SUCCESS);

        const uint16_t test_port = (family == AF_INET) ? 30411 : 30412;
        SOCKET socket_handle = WSASocket(family, SOCK_DGRAM, IPPROTO_UDP, nullptr, 0, 0);
        REQUIRE(socket_handle != INVALID_SOCKET);
        SOCKADDR_STORAGE sock_addr{};
        sock_addr.ss_family = static_cast<ADDRESS_FAMILY>(family);
        INETADDR_SETANY((PSOCKADDR)&sock_addr);
        SS_PORT(&sock_addr) = htons(test_port);
        REQUIRE(bind(socket_handle, (PSOCKADDR)&sock_addr, sizeof(sock_addr)) == 0); // PROCEED_SOFT => succeeds.

        // Poll for the ring buffer event generated by the real bind.
        bool found = false;
        for (int i = 0; i < 25 && !found; i++) {
            REQUIRE(ring_buffer__poll(ring, 200) >= 0);
            for (auto port : event_context.ports) {
                if (port == htons(test_port)) {
                    found = true;
                    break;
                }
            }
    ...
    bind_and_expect_event("authorize_bind4", EBPF_ATTACH_TYPE_CGROUP_INET4_BIND, AF_INET);
    bind_and_expect_event("authorize_bind6", EBPF_ATTACH_TYPE_CGROUP_INET6_BIND, AF_INET6);
