    ...
    while (nullptr !=
        ...
        if (ebpf_ring_buffer_record_is_discarded(record)) {
            ...
            if (mapping.is_perf_buffer) {
                ...
                callback_result = ((ring_buffer_sample_fn)mapping.sample_fn)(
                    mapping.ctx,
                    (void*)record->data, // Point to data portion of record.
                    data_length);
            }
    ...
    try {
        ...
        if (perf_buffer->is_async_mode) {
            ...
            for (uint32_t cpu_id = 0; cpu_id < cpu_count; cpu_id++) {
                ...
                try {
                    perf_buffer->sync_maps.push_back(map_info);
                } catch (const std::bad_alloc&) {
                    CloseHandle(wait_handle);
                    perf_buffer->wait_handle = ebpf_handle_invalid;
                    (void)ebpf_ring_buffer_map_unmap_buffer(
...
perf_buffer__poll(struct perf_buffer* pb, int timeout_ms)
{
    ...
    // First a non-blocking wait to clear the auto-reset event if already set.
    DWORD wait_result = WaitForSingleObject(reinterpret_cast<HANDLE>(pb->wait_handle), 0);

    // Next, try to consume any immediately available data.
    int total_records = perf_buffer__consume(pb);
    if (total_records != 0) {
        return total_records; // Return records found or error.
    }
